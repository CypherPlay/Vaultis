...
describe('GuessHistory', () => {
	...
	it('renders a list of guesses with pagination', async () => {
		...
		expect(
			screen.getByText(new Date(mockGuesses[0].timestamp).toLocaleString())
		).toBeInTheDocument();
		expect(
			screen.queryByText(new Date(mockGuesses[5].timestamp).toLocaleString())
		...
		// Check page 2
		expect(
			screen.queryByText(new Date(mockGuesses[0].timestamp).toLocaleString())
		).not.toBeInTheDocument();
		expect(
			screen.getByText(new Date(mockGuesses[5].timestamp).toLocaleString())
		).toBeInTheDocument();
		...
		// Check last page
		expect(
			screen.getByText(new Date(mockGuesses[10].timestamp).toLocaleString())
		).toBeInTheDocument();
		expect(
			screen.queryByText(new Date(mockGuesses[9].timestamp).toLocaleString())
		).not.toBeInTheDocument();
	});
