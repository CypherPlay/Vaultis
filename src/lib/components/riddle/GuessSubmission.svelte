<script lang="ts">
	...
	import { submitGuess } from '$lib/utils/apiClient';
	...
	async function handleSubmit() {
		...
		try {
			const response = await submitGuess({ walletAddress, guess: trimmedGuess });
...
{:else}
	<div class="p-4 rounded-lg shadow-md bg-white border max-w-md mx-auto">
		...
		<form on:submit|preventDefault={handleSubmit}>
			...
			<button
				type="submit"
				class="py-2 px-4 rounded-md shadow-sm text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-700 focus:ring-indigo-500 flex w-full items-center justify-center border border-transparent focus:ring-2 focus:ring-offset-2 focus:outline-none"
				disabled={loading}
				aria-busy={loading}
			>
				{#if loading}
